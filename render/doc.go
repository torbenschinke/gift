...
// ([Op]), the colour convention ([Color]) and the [Backend] contract that a
// concrete renderer such as backend/ebiten implements. It deliberately
...
// start of the next frame. See [List.Ops] and [Backend.Submit].
//
// # Completeness
//
// Step 1 covers rectangles, rounded rectangles and strokes together with
// clipping and transforms. Text, images and material regions are part of
// later steps. [Op] is a struct with a discriminating [OpKind] precisely so
// that those can be added without changing the shape of the list.
package render
