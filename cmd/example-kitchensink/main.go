...
// app is the handle the theme switch needs; see cmd/example-counter, which
// makes the same compromise for the same reason.
var app *gift.App

// pictures are the sources the Media tab shows. They are generated into the
// user's cache directory on first run, so the example needs no assets and no
// network and still exercises the real pipeline: probe, decode, thumbnail
// ladder, disk cache and GPU residency.
